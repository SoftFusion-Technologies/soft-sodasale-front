// src/Components/Geografia/LocalidadCard.jsx
...
export default function LocalidadCard({
  ...
  const initial = useMemo(
    () => (item?.nombre ? item.nombre[0]?.toUpperCase() : 'L'),
    [item?.nombre]
  );
  const inactiva = item?.estado !== 'activa';

  // Props para el DetailViewModal (misma filosofía que ProductCard/CiudadCard)
  const buildViewProps = (l) => {
    const activa = (l?.estado || '').toLowerCase() === 'activa';
    return {
      title: l?.nombre || 'Localidad',
      subtitle: l?.id ? `ID ${l.id}` : undefined,
      icon: FaMapMarkerAlt,
      leftAccent: color,
      status: {
      ...
      sections: [
        ...
        },
        {
          ...
          cols: 2,
          rows: [
            { label: 'Ciudad', value: l?.ciudad?.nombre ?? '—' },
            {
              label: 'Provincia',
              value: l?.ciudad?.provincia ?? '—'
            }
          ]
        }
      ],
        ...
        >
          ...
          <div className="flex items-start sm:items-center gap-4">
            <div className="relative -ml-2 sm:ml-0 h-14 w-14 shrink-0 rounded-2xl ring-1 ring-white/30 bg-white/90 dark:bg-zinc-800/80 flex items-center justify-center">
              ...
              <FaMapMarkerAlt className="absolute -right-2 -bottom-2 text-white" />
            </div>
            <div className="min-w-0">
              ...
              <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-400">
                {item?.ciudad?.nombre}{' '}
                {item?.ciudad?.provincia ? `• ${item.ciudad.provincia}` : ''} •
                ID {item?.id}
              </div>
            </div>
          </div>
          ...
          <div className="min-w-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Field label="Ciudad">{item?.ciudad?.nombre}</Field>
              <Field label="Estado">
