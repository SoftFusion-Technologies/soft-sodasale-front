...
export default function CiudadCard({
  ...
  // Props para el DetailViewModal estándar (igual filosofía que ProductCard)
  const buildViewProps = (c) => {
    const activa = (c?.estado || '').toLowerCase() === 'activa';
    return {
      title: c?.nombre || 'Ciudad',
      subtitle: c?.id ? `ID ${c.id}` : undefined,
      icon: FaCity,
      leftAccent: color,
      status: {
        ...
        >
          ...
          <div className="min-w-0">
            ...
            <div className="mt-4 grid grid-cols-[repeat(auto-fit,minmax(120px,1fr))] gap-2">
              {/* Ver detalle (igual que ProductCard: usa onView si viene, sino modal interna) */}
              <button
