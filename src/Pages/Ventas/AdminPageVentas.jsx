...
const AdminPageVentas = () => {
  ...
  const handleNuevaVenta = async ({ venta, items }) => {
    try {
      ...
      // acá más adelante podés refrescar el listado si lo tenés en esta página
      // ej: reloadVentas();
      ...
      // NO cerramos el modal, así el user corrige
      throw err;
    } finally {
      setCreating(false);
    }
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 justify-center">
              {ventasLinks.map(({ to, label, icon }, index) => {
                ...
                // 👉 Tarjeta especial: abre modal en vez de navegar
                if (isNuevaVenta) {
                  return (
                    <button
                      type="button"
